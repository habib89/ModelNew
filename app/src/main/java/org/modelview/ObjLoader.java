...
public class ObjLoader {
	...
	private static HashMap<String, Bitmap> textures = new HashMap<String, Bitmap>();

	private static Pattern whitespacePattern = Pattern.compile("\\s+");
	private static Pattern facePattern = Pattern.compile("(\\d+)?/(\\d+)?/(\\d+)?");
//	private static HashMap<String, InputStream> map = new HashMap<String, InputStream> ();
	private static ObjFace parseFace(String v1, String v2, String v3, String material) throws IOException {
//
		Matcher matcher;
			...
			while ((line = reader.readLine()) != null)
			{
				...
				if (tokens[0].equals("newmtl")) {
					...
					if (currentMaterial.texture == null) {
						...
						Bitmap m = textures.get(textureFileName);
						if(m != null) {
							currentMaterial.texture = m;
//							Log.d("KAMBINGGGG", "AMBIL DARI MAP "+a + textureFileName);
						} else {
							currentMaterial.texture = BitmapFactory.decodeStream(ctx.getAssets().open(textureFileName));
							textures.put(textureFileName, currentMaterial.texture);
//							Log.d("KAMBINGGGG", "BUAT BARU SIMPAN KE MAP "+a + textureFileName);
						}

//						InputStream is = map.get(tokens[1]);
